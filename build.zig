...
pub fn build(b: *std.Build) void {
    ...
    // Modular build system - export individual modules
    // Users can import only what they need for smaller binaries and faster compilation

    // Core QUIC protocol module
    ...
    client_module.addImport("core", core_module);
    ...
    server_module.addImport("core", core_module);

    // Crypto module (for QUIC encryption/decryption)
    ...
    // Main module that includes everything (for convenience)
    const mod = b.addModule("quic-zig", .{
    ...
    mod.addImport("client", client_module);
    mod.addImport("server", server_module);
    mod.addImport("crypto", crypto_module);
    mod.addImport("utils", utils_module);
    ...
    const exe = b.addExecutable(.{
        ...
        .root_module = b.createModule(.{
            ...
            // root module.
            .imports = &.{
                // Here "quic_zig" is the name you will use in your source code to
                // import this module (e.g. `@import("quic_zig")`). The name is
                // repeated because you are allowed to rename your imports, which
                // can be extremely useful in case of collisions (which can happen
                // importing modules from different packages).
                .{ .name = "quic_zig", .module = mod },
            },
        }),
    });
    ...
    // Build server mode executable
    const server_exe = b.addExecutable(.{
        .name = "quic_server",
        .root_module = b.createModule(.{
            .root_source_file = b.path("src/server_bin.zig"),
            .target = target,
            .optimize = optimize,
            .imports = &.{
                .{ .name = "quic_zig", .module = mod },
    ...
    // Build client mode executable
    const client_exe = b.addExecutable(.{
        .name = "quic_client",
        .root_module = b.createModule(.{
            .root_source_file = b.path("src/client_bin.zig"),
            .target = target,
            .optimize = optimize,
            .imports = &.{
                .{ .name = "quic_zig", .module = mod },
    ...
    // Build runner mode (interop test coordinator)
    ...
    // Build endpoint for Docker/interop runner
    const endpoint_exe = b.addExecutable(.{
        .name = "quic_endpoint",
        .root_module = b.createModule(.{
    ...
    b.installArtifact(endpoint_exe);
    ...
    // Run real QUIC interop tests (delegates to flake command)
    const runner_step = b.step("runner", "Run QUIC interop tests (use: test-interop from nix shell)");
